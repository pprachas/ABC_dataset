...
#---------------------Note-----------------------------------------#
#This code is the code for pointnet layers used in the manuscript
...
class PointNet4Layers(torch.nn.Module):
    ...
    def forward(self, pos, x, batch, r):
        # For the following lines regarding edge index, only use one depending on the use case
        
        # Use the line below for ball radius (overwrite the edge indices with ball query + ball indices)
        # To copy the manuscript, r = 20, 30, or 40 
        edge_index = radius_graph(pos.float(),r=r,batch=batch,loop=True)
        
        
        # Use line below for graphs RAG representations and exact structures (use the same edge indices + self loop) 
